...
zeroResultsRate_all %>% mutate(day = day(day)) %>%
   ...
   ggplot(zeroResultsRate_all,
    mapping = aes(x = day, y = zero_results_prop)) + geom_point(size = 3, colour="red") + geom_line() + labs(y =
...
zeroResultsRate  %>% mutate(day = day(day)) %>%
    ...
    group), size = 0.5) + labs(y = "Taxa de Resultados Zero", x = "Dias", colour = "Grupo")



    ggplot(zeroResultsRate, mapping = aes(x = group, y = zero_results_prop))   +
  geom_boxplot(coef = 1000, width = .5) + labs(y="Taxa de Resultados Zero", x="Grupos")
